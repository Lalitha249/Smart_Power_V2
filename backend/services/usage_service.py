...
from models.usage_model import (insert_usage_log , get_total_usage , get_device_usage_by_month)
from models.subscription_model import get_subscription, get_plan
from models.user_model import find_user_by_id
from services.reward_service import update_user_rewards
...
def add_usage_service(data: dict):
    ...
    if subscription.get("status") != "active":
       return {"error": "Subscription not active"}, 403
    # ---------------- CHECK USER ----------------
...
def get_usage_summary_service(user_id: str):
    ...
    #print("DEBUG included_units:", included_units)
    ...
    reward_points = update_user_rewards( user_id, total_units, included_units)
    return {
...
# -------------------------------------------------
def get_device_analytics_service(user_id: str):
    ...
    device_data = get_device_usage_by_month(user_id, month)

    # format response cleanly
    formatted = [
        {
            "device_name": item["_id"],
            "total_units": item["total_units"]
        }
        for item in device_data
    ]

    highest_device = formatted[0] if formatted else None

    return {
        "devices": formatted,
        "highest_usage_device": highest_device
    }, 200